...
def lost_memory_report():
    ...
    if admin_email and EMAIL_ENABLED and email_manager.gateway.is_configured():
        try:
            ...
            def send_email_bg():
                ...
                loop.run_until_complete(
                    email_manager.send_memory_report(client_id, report, admin_email)
                )
                loop.close()
    ...
    elif admin_email and not EMAIL_ENABLED:
    ...
    return jsonify({
        ...
        "email_disabled": not EMAIL_ENABLED
    })
...
def test_email():
    ...
    test_email = data.get("email", os.environ.get("ADMIN_EMAIL"))
    if not test_email:
    ...
    if not EMAIL_ENABLED:
        return jsonify({
            ...
            "details": "Email functionality has been temporarily disabled but will be re-enabled in the future",
            "email": test_email,
            "feature_disabled": True
        }), 400
    
    if not email_manager.gateway.is_configured():
        return jsonify({
            "error": "ProtonMail not configured",
            "details": "Set PROTONMAIL_API_KEY environment variable"
        }), 400
    ...
    try:
        ...
        def send_test_email():
            ...
            result = loop.run_until_complete(
                email_manager.gateway.send_notification(
                    test_email,
                    "ICI Chat Email Test",
                    f"""
...
Server: Cloud Run ProtonMail Gateway

If you received this email, the ProtonMail integration is working correctly!
                    """
                )
            )
            loop.close()
        ...
        def email_thread():
            result_container['result'] = send_test_email()
        
        thread = threading.Thread(target=email_thread, daemon=True)
        thread.start()
        ...
        if 'result' in result_container:
            ...
            return jsonify({
                ...
                "message": "Test email sent" if result.get('success') else "Email failed",
                "details": result,
                "email": test_email
            })
        else:
...
def email_status():
    ...
    return jsonify({
        "configured": EMAIL_ENABLED and email_manager.gateway.is_configured(),
        "feature_enabled": EMAIL_ENABLED,
        "api_key_set": bool(os.environ.get('PROTONMAIL_API_KEY')) if EMAIL_ENABLED else False,
        "admin_email_set": bool(os.environ.get('ADMIN_EMAIL')),
        "sender_email": email_manager.gateway.sender_email,
        "admin_email": os.environ.get('ADMIN_EMAIL', 'Not set'),
        "status_message": "Email features are currently disabled" if not EMAIL_ENABLED else "Email features are enabled"
    })
